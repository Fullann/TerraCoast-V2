...
export function LeaderboardPage({ onNavigate }: LeaderboardPageProps = {}) {
      ...
      const enrichedData = await Promise.all(
        profiles.map(async (p) => {
          if (period === "monthly") {
            ...
            const archivedScore =
              monthlyHistory?.reduce((sum, m) => sum + m.score, 0) || 0;
            const archivedGames =
              monthlyHistory?.reduce((sum, m) => sum + m.games_played, 0) || 0;

            // Ajouter le score du mois actuel
            const currentMonthScore = p.monthly_score || 0;
            const currentMonthGames = p.monthly_games_played || 0;

            const total_score = archivedScore + currentMonthScore;
            const games_played = archivedGames + currentMonthGames;
  ...
  const getRankBackground = (index: number) => {
    ...
    if (index < 10 && period === "monthly")
      return "bg-gradient-to-r from-emerald-50 to-teal-50 border-emerald-300";
          ...
          {leaderboard.map((entry, index) => (
            <div
              ...
              className={`${getRankBackground(
                index
              )} rounded-xl border-2 p-6 transition-all hover:shadow-lg cursor-pointer`}
            >
