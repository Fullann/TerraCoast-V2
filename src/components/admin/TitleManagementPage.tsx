    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-red-50 border-2 border-red-200 rounded-xl p-8 text-center">
          ...
          <p className="text-gray-600">Seuls les administrateurs peuvent gérer les titres</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                >
                  ...
                  <option value="account_age_days">Âge du compte (jours)</option>
              ...
              <div className="flex items-center pt-6">
                ...
                />
                <label htmlFor="is_special" className="ml-2 text-sm text-gray-700">
                  Titre spécial
                </label>
              </div>
            </div>
          ...
          {titles.map((title) => (
            <div
              ...
              className={`rounded-xl shadow-md p-6 hover:shadow-xl transition-shadow ${
                title.is_special ? 'bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-300' : 'bg-white'
              }`}
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-3">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                    ...
                    <Star className={`w-6 h-6 ${title.is_special ? 'text-amber-700 fill-amber-700' : 'text-amber-600'}`} />
                  </div>
                  <div>
              ...
              <div className="bg-gray-50 rounded-lg p-3 mb-4">
                ...
                <p className="text-sm text-gray-800">
                  {title.requirement_type === 'level' && `Niveau ${title.requirement_value}`}
                  {title.requirement_type === 'first_place' && `${title.requirement_value} première place`}
                  {title.requirement_type === 'account_age_days' && `Compte de ${title.requirement_value} jours`}
                  {title.requirement_type === 'wins' && `${title.requirement_value} victoires`}
                  {title.requirement_type === 'quizzes_completed' && `${title.requirement_value} quiz complétés`}
                  {title.requirement_type === 'perfect_scores' && `${title.requirement_value} scores parfaits`}
                </p>
              </div>
      ...
      {!loading && titles.length === 0 && !isCreating && (
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">Aucun titre</h3>
